...
pfactors<-function(data.factors, palette="FantasticFox1")
{

  for(i in 1:ncol(data.factors)){
    dt<-table(data.factors)
    par(mar=c(1,2,1,0.5))
    barplot(as.matrix(dt), beside=F,legend.text = names(dt), col=getcolhabs(palette,length(dt)), main=colnames(data.factors)[i],las=1)
  }
  plofac<-recordPlot()
  return(plofac)
...
## plot RDA
prda<-function(model,pch=c(16,3),key=NULL,keytext=NULL,points=T,text=F,palette="black",col.sp="red",col.arrow='steelblue', show.sp=T, scaling=1, constr=FALSE, cex.bp=1,col.lab="red",cex.text=.8,cex.points=1,textcolor="gray", biplot=T)
{
  if(!is.null(key)) {
    colkey<-getcolhabs(palette, nlevels(key))
    col<-colkey[key]} else{col= getcolhabs(palette, nrow(model$CA$Xbar)) }
  opar<-par(no.readonly=TRUE)
  layout(matrix(c(1,2), nrow=1),widths = c(100,20))
  par(pty = "s",mar=c(5,5,5,1))
...
xlab<-paste("RDA1 -"," (",labs[1]," %", ")*", sep="")
ylab<-paste("RDA2 -"," (",labs[2]," %", ")*", sep="")
xlab<-paste("RDA I")
ylab<-paste("RDA II")
si<-scores(model,display="sites")
bp<-scores(model,display="bp")
sp<-scores(model,display="sp")
plotrda<-plot(model,pch=pch[1],xlab=xlab,ylab=ylab,col=col,type="n", las=1)
if(isTRUE(biplot)){
if(length(bp)>0){
points(model, display="bp", axis.bp =T)
text(model,display="bp", axis.bp =T)
}}
if(isTRUE(points)){points(model, display="sites", cex=cex.points, col=col, pch=pch[1])}
if(isTRUE(text)){text(model, display="sites",labels=keytext, cex=cex.text,  col=textcolor)}


if(isTRUE(show.sp)){points(sp, pch=3,col=col.sp)}
if(!is.null(key)) {
  par(mar=c(0,0,0,0))
  plot.new()
  colkey<-getcolhabs(palette, nlevels(key))
