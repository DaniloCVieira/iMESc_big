list.of.packages <- c('shinydashboard','shinydashboardPlus','shinyjs','shiny',"e1071",'readxl','vegan',"party",'caret','viridisLite','aweSOM','sp','raster','Rcpp','rgdal','gstat','ggspatial','ggplot2','sf','class','shinyWidgets', 'randomForestExplainer','data.table',"ggpubr", "shinyBS","terra","purrr","NbClust", "colorRamps","DBI","shinyBS","wesanderson","colorspace","gplots","dendextend","kohonen","shinypanels","writexl","DT","gbRd", 'segRDA',"shinyjqui")
...
sidebarMenu(
  ...
  div(
    ...
    menuItem(
      div(
        icon("fas fa-tree"),
        icon("fas fa-dyalog", style = "margin-left: -20px;  font-size: 14px"),
        HTML('&nbsp;'),
        "Decision",
      ...
      div(style = "margin-left: -15px; margin-right:-15px;",
          column(12,
            splitLayout(cellWidths = c("90%","10%"),
              uiOutput("menutitle"),actionButton("dialog",strong("Savepoints"),style="font-size: 10px; background: SeaGreen; color: white", icon=icon("fas fa-download"))
            ),
...
output$fac_levels<- renderUI({
  ...
  output$stats_classmat<-renderUI({
    data<-saved_data$df[[input$data_upload]]
    ...
    column(12,style="background: white",
           column(12,
                  ...
                  column(12,"A Data-Attribute can handle only numeric variables. Use the tools bellow to explore and include the ommited factors as numeric variables"),
                    ...
                    fluidRow(
                      column(12,
                             splitLayout(cellWidths = c('25%','25%',"50%"),
                                         selectInput("sel_ommitedfac", strong("Factor", pophelp(NULL, "Select the factor to see its count")),choices=colnames(attr(data,"data.factors")), selectize=F),
                                         column(12,
                                                strong("Palette"),
                                                pickerInput(inputId = "fac_palette",
                                                            label = NULL,
                                                            choices = colors_gradient$val,
                                                            choicesOpt = list(content = colors_gradient$img), options=list(container="body")))
                                         ,
                             ...
                             renderPlot(pfactors(
                               attr(data, 'data.factors')[input$sel_ommitedfac], palette=input$fac_palette
                             ))
                      )
                    )
                  }),
  ...
  bag_levels<-reactiveValues(df=0)
...
new_facts<-reactiveValues(df=NULL)
...
getres<-reactive({
  data=attr(saved_data$df[[input$data_upload]],"data.factors")
  res<-getlevels()[input[[names(input)[grep("_order", names(input))]]],]
  res<-factor(data[,input$to_ordinal], levels=res, labels=1:length(res))
  res<-as.numeric(res)
  res
})
get_newlevels<-eventReactive(input$create_ordinal,{
data=attr(saved_data$df[[input$data_upload]],"data.factors")
 res=getres()
  if(is.null( new_facts$df)){
    res0<-data.frame(matrix(NA,nrow=nrow(data)))
    colnames(res0)<-input$to_ordinal
    rownames(res0)<-rownames(data)
    new_facts$df<-res0
    }
  new_facts$df[input$to_ordinal]<-res
...
output$binary_preview<-renderUI({
  ...
  observeEvent(input$insert_ordinal,{
    hand_save$df<-"Include ordinal variables in the Data-Attribute"
    showModal(
      hand_save_modal()
    )})
  ...
  observe({
    req(hand_save$df=="Create factor using breakpoints from the dissimilarity profile")
  ...
  observeEvent( input$data_confirm,{
    ...
    if(hand_save$df=="Include ordinal variables in the Data-Attribute"){
      data=saved_data$df[[input$data_upload]]
      temp<-cbind(data,new_facts$df)
    ...
    if(hand_save$df=="Create factor using breakpoints from the dissimilarity profile"){
      bagbp0$df<-bagbp0$df+1
      dp<-getDP()

      if(input$hand_save=="create"){
        attr(saved_data$df[[input$data_upload]],"factors")[rownames(splitBP$df),input$bp_newname]<-as.vector(splitBP$df)
        attr(saved_data$df[[input$data_upload]],"factors")[,input$bp_newname]<-as.factor(attr(saved_data$df[[input$data_upload]],"factors")[,input$bp_newname])

      } else{
        attr(saved_data$df[[input$data_upload]],"factors")[input$bp_over]<-as.vector(splitBP$df)
        attr(saved_data$df[[input$data_upload]],"factors")[,input$bp_over]<-as.factor(attr(saved_data$df[[input$data_upload]],"factors")[,input$bp_over])

        }

    }

    if(hand_save$df=="Save changes"|hand_save$df=="Include binary columns in the Data-Attribute"|hand_save$df=="Include ordinal variables in the Data-Attribute"){
  ...
  output$data_create<-renderUI({
    ...
    if(hand_save$df=="Save newsom in"){textInput("model_newname", NULL,paste("Som", length(attr(getdata_som(),"som"))+1))} else
      if(hand_save$df=="Save changes"|hand_save$df=="Include binary columns in the Data-Attribute"|hand_save$df=="Include binary columns in the Data-Attribute"|hand_save$df=="Include ordinal variables in the Data-Attribute"){
        bag<-nrow(attr(getdata_upload(), "transf") )
        if(!length(bag)>0){bag<-1}
        name0<-paste("Datalist_",gsub(".csv","",attr(getdata_upload(), "filename")))
        name1<-paste(name0,bag)
        if(name1%in%names(saved_data$df))
        {
          repeat{
            bag<-bag+1
            name1<-paste(name0,bag)
            if(!name1%in%names(saved_data$df)) break
          }
      ...
      textInput("data_newname", NULL,paste("Datalist_",gsub(".csv","",attr(getdata_upload(), "filename")) ,bag))} else
        ...
        textInput("div_newname", NULL,paste("Div_results", length(saved_data$df)+1))}else
          ...
          } else if(  hand_save$df=="Create factor using breakpoints from the dissimilarity profile"){textInput("bp_newname", NULL,paste("bp", bagbp0$df+1))}
  ...
  output$data_over<-renderUI({
    ...
    }else if(hand_save$df=="Save changes"|hand_save$df=="Include binary columns in the Data-Attribute"|hand_save$df=="Include ordinal variables in the Data-Attribute") { selectInput("data_over", NULL,choices=c(names(saved_data$df)),selectize = F)} else if(hand_save$df=="Save Clusters"){
    ...
    } else if(hand_save$df=="Create factor using breakpoints from the dissimilarity profile"){
      selectInput("bp_over", NULL,choices=c(colnames(attr(saved_data$df[[input$data_upload]],"factors"))),selectize = F)
    }
  ...
  output$preview<-renderUI({
    ...
    } else if(hand_save$df=="Include binary columns in the Data-Attribute"|hand_save$df=="Include ordinal variables in the Data-Attribute"){
      data<-saved_data$df[[input$data_upload]]
...
output$shp_create<-renderUI({
  ...
  output$editbox<-renderUI({
    res<-na.omit(getbox())
    fluidRow(style="border: 3px solid SeaGreen;background: white; margin-bottom: 20px; margin-left: -20px; margin-right: -40px",
           ...
           tabsetPanel(
             ...
             tabPanel(
               ...
               tabsetPanel(
                 ...
                 tabPanel(strong("2.2 Training error "), verbatimTextOutput("rf_table")),
    ...
    column(
      ...
      tabsetPanel(id="som_res",
                  ...
                  tabPanel(value= 'train_tab1',
                           ...
                           column(12, style = "background: white;",
                                  splitLayout(
                                    verbatimTextOutput("train.summary"),
                                    uiOutput("error_som")
  ...
  labels <- reactive({
    ...
    if (input$up_or_ex == 'upload') {
      ...
      labels <-data.frame(fread("meta/factors_araca.csv", stringsAsFactors = T))
  ...
  output$menu_upload_out<-renderUI({
    ...
    fluidRow(
      column(12, style="margin-left: 10px",
             ...
             if(input$tabs=="menu_upload"){ uiOutput("upload_side")}
             else{
               fluidRow(
                 ...
                 column(12,style="margin-top: -15px; margin-left: -5px",
                        uiOutput("panel_main")),
...
observeEvent(input$desc_options,{
  bag_submenu$df<-switch(input$desc_options,
                         'Boxplot'="boxplot",
                         'MDS'="mds",
                         'RDA'="rda",
                         'PCA'="pca")
...
output$panel_main<-renderUI({
  ...
  column(12,style=paste(border_alert,"; background: white"),
         column(12,
                if(length(attr(data,"data.factors"))>0){
                  radioGroupButtons("desc_options",NULL,choices=c("Warning","Summaries","Histogram","Boxplot","MDS","PCA","RDA","segRDA"), status  ="button_active",justified =T,direction="horizontal")
                }else{
                  radioGroupButtons("desc_options",NULL,choices=c("Summaries","Histogram","Boxplot","MDS","PCA","RDA","segRDA"), status  ="button_active",justified =T,direction="horizontal")
                }),
         conditionalPanel("input.desc_options=='Warning'",{uiOutput("stats_classmat")}),

         conditionalPanel("input.desc_options=='Summaries'",{
           column(12,splitLayout(
             cellWidths = c('10%','90%'),
             column(12,fluidRow(style="margin-top: 15px",
                                radioGroupButtons("summ_options",NULL,choices=c("Data","Variables","Factors"), status  ="button_active",justified =F,direction="vertical")
             )),
             uiOutput("summ_out")
           ))
         }),

         conditionalPanel("input.desc_options=='Histogram'",{uiOutput("stats_chist")}),
         conditionalPanel("input.desc_options=='Boxplot'",{
           column(12,uiOutput("stats_cbox"))
         }),
         conditionalPanel("input.desc_options=='MDS'",{
           column(12,uiOutput("stats_cmds"))
         }),
         conditionalPanel("input.desc_options=='PCA'",{
           column(12,uiOutput("stats_cpca"))
         }),
         conditionalPanel("input.desc_options=='RDA'",{
           column(12,uiOutput("stats_crda"))
         }),
         conditionalPanel("input.desc_options=='segRDA'",{
           column(12,uiOutput("stats_csegrda"))
           }),
         conditionalPanel("input.desc_options=='Boxplot'|input.desc_options=='MDS'|input.desc_options=='PCA'|input.desc_options=='RDA'",{
           uiOutput("submenu_tools")
         }),
         conditionalPanel("input.desc_options=='Histogram'",{uiOutput("stats_phist")}),
         conditionalPanel("input.desc_options=='Boxplot'",{
           column(12,uiOutput("stats_pbox"))
         }),
         conditionalPanel("input.desc_options=='MDS'",{
           column(12,uiOutput("stats_pmds"))
         }),
         conditionalPanel("input.desc_options=='PCA'",{
           column(12,uiOutput("stats_ppca"))
         }),
         conditionalPanel("input.desc_options=='RDA'",{
           column(12,uiOutput("stats_rda"))
         }),
         conditionalPanel("input.desc_options=='segRDA'",{
           column(12,uiOutput("stats_segrda"))
         })
  )
  ...
  output$stats_cpca<-renderUI({
    column(12,style="background: white",
           ...
           column(12,checkboxInput('biplot', strong("biplot", pophelp(NULL,"show biplot arrows")), T))
    )
  })
  output$stats_cmds<-renderUI({
    column(12,style="background: white",
           p(strong("Nonmetric Multidimensional Scaling ")),
           p(strong("distance measure")),
           p(uiOutput("distance_mds"))
    )
  })
  output$stats_crda<-renderUI({
    validate(need(length(saved_data$df)>1, "This functionality requires at least two datalist as explanatory and response data."))
    column(12,style="background: white",
           p(style="margin-top:20px",strong("Redundancy Analysis")),
           p(
             splitLayout(
               cellWidths = c("60%","40%"),
               column(12,
                 p(
                   strong("Y Data:", style="color: #0D47A1"),em(input$data_upload),strong("~", style="color: #0D47A1"))
               ),
               column(12,uiOutput("rda_X"))



             ),
             column(12,style="margin-top: -70px",checkboxInput("rda_scale",strong("scale variables",tiphelp("Scale variables to unit variance (like correlations)")), value=T))
           )
    )
  })
    ...
    column(12,style="background: white",
           ...
           p(
             splitLayout(
               ...
               column(12,uiOutput("segrda_X"))

             ),
             column(12,style="margin-top: -60px; margin-botton: -20px",
                    p(checkboxInput("segrda_scale",strong("scale variables",tiphelp("Scale variables to unit variance (like correlations)")), value=T)),
                    p(style="margin-top: -20px",splitLayout(cellWidths = c("25%","20%"),
                      checkboxInput("segrda_ord",strong("Data ordination",pophelp(NULL,"Both the SMW and pwRDA analyses depend on ordered datasets. Defaults to ordering both response and explanatory matrices using one of the axes of the RDA model. If unchecked,please make sure all your inputs are already ordered.")), value=T),
                      uiOutput("ord_check")
                    ))
             )
           )
...
output$ord_check<-renderUI({
  ...
  {
    p(numericInput("axis_ord_segrda","axis", value=1, step=1))
  } else{
...

  {
    ...
    observeEvent(input$revert_rda,{
      ...
      updateSelectInput(session,"data_upload",NULL,choices=names(saved_data$df), selected=input$rda_X)
      updateSelectInput(session,"rda_X",strong("Y Data", tiphelp("Response data")), choices=names(saved_data$df)[-which(names(saved_data$df)%in%input$data_upload)],selected=cur)
    ...
    output$rda_X<-renderUI({
      if(length(saved_data$df)>1){
        fluidRow(
          p(style="margin-left: -20px",span(actionLink("revert_rda",tipify(icon("fas fa-exchange-alt"),"invert selection")), align="right"),strong("X Data", tiphelp("Response data"))),
          selectInput("rda_X",NULL, choices=names(saved_data$df)[-which(names(saved_data$df)%in%input$data_upload)], selectize=F)
        )
      }else{em(icon("fas fa-lightbulb"),"you can create a new datalist to use it as a response data")}
    })
    rda_model<-reactive({
      data<-getdata_upload()
      x<-as.matrix(data)
      ...
      if(length(input$rda_X)>0){
        y<-na.omit(saved_data$df[[input$rda_X]][rownames(x),,drop=F])
        colnames(y)<-colnames(saved_data$df[[input$rda_X]])
        x<-na.omit(x[rownames(y),])
        model=rda(x~.,data=data.frame(y) ,scale=input$rda_scale)
      } else{model= rda(x,scale=input$rda_scale)}
      model})
    rda_plot<-reactiveValues(df=0)
      ...
      fluidRow(
        tabsetPanel(
          tabPanel("Plot",
                   column(5,
                          splitLayout(
                            checkboxInput('biplot_rda', strong("biplot", pophelp(NULL,"show biplot arrows")), T),
                            checkboxInput('sp_rda', strong("variables", pophelp(NULL,"show variables")), T)
                          )
                         ),
                   renderPlot({
                     if(length(input$show_symbols)>0){
                       ...
                       rda=prda(rda_model(), biplot=input$biplot_rda, show.sp=input$sp_rda)
                     }
                     rda_plot$df<-recordPlot()
                     rda

                   })),
          tabPanel("Summary",
                   column(12,
                          splitLayout(cellWidths = c("60%","25%","15%"),
                                      column(12,
                                             column(12,strong("Display"),style="color: #0D47A1"),
                                             column(12,selectInput("disp_rda_summ",NULL,choices=c('Variable scores','Observation scores','Linear constraints','Biplot','Importance (unconstrained)',"Importance (constrained)"), selectize= F, selected="Importance (unconstrained)"))
                                      ),
                                      column(12,numericInput("rda_axes","Number of axes", value=2)),
                                      column(12,style="margin-top: 25px",tipify(actionButton("downcenter_rda",icon("fas fa-download"), icon("fas fa-table")),"Download selected results", options=list(container="body")))

                          ),


                   ),
                   column(12,verbatimTextOutput("rda_print"))
...

  {
    ...
    output$stats_segrda<-renderUI({
      validate(need(!anyNA(getdata_upload()), "This functionality does not support missing values; Please use the transformation tool to the handle missing values."))

      column(12,style="background: white;",
                        column(12,uiOutput("segrda_out"))
      )
    })
getord<-reactive({
  ...
  colnames(x)<-colnames(getdata_upload())
  y<-na.omit(as.matrix(saved_data$df[[input$segrda_X]][rownames(x),,drop=F]))
  colnames(y)<-colnames(saved_data$df[[input$segrda_X]])
  x<-na.omit(x[rownames(y),])
  if(isTRUE(input$segrda_ord)){
...
req(isTRUE(input$segrda_ord))
    ...
    segrda_windows<-reactiveValues(df=NULL)
    ...
    observeEvent(input$include_window,{
      segrda_windows$df<-c(segrda_windows$df,input$segrda_w)
    })

    observeEvent(input$remove_windows,{
      segrda_windows$df<-NULL
    })
    output$smw_create<-renderUI({
      splitLayout(cellWidths = c('20%','15%','50%',"15%"),
                  p(
                    p(strong("window",style="color: #0D47A1")),
                    numericInput("segrda_w",NULL,value=if(is.null(segrda_windows$df)){10}else{segrda_windows$df[length(segrda_windows$df)]+2}, step=2)
                  ),
                  p(p(HTML('&nbsp;')),
                    actionButton("include_window",tipify(icon("fas fa-arrow-right"),"Include window size in the pool"), style="button_active")
                  ),
                  p(
                    p(strong("Pool",style="color: #0D47A1")),
                    uiOutput("segrda_windows")
                  ),
                  p(p(HTML('&nbsp;')),
    ...
    output$segrda_smw<-renderUI({
      column(12,
             radioGroupButtons("smw_default","Window Pool",choices=c("Default","Create")),
             conditionalPanel("input.smw_default=='Create'",{
               uiOutput('smw_create')
             }),
             conditionalPanel("input.smw_default=='Default'",{
               uiOutput('smw_default')
             }),
             column(12,splitLayout(
               selectInput("smw_dist",strong("dist", tiphelp("Dissimilarity index")), choices=c("bray","euclidean","manhattan","jaccard"), selectize = F),
               selectInput("smw_rand",strong("rand",tipify(actionLink('smw_rand_help',icon("fas fa-question-circle")),"The type of randomization for significance computation. Click for details")), choices=c("shift","plot"), selectize = F),
               numericInput("smw_nrand",strong('n.rand', tiphelp("The number of randomizations")), value=10),
               p(style="margin-top: 25px",actionButton("go_smw",strong("run SMW"), style="button_active"))
    ...
    plot_we<-reactiveValues(df=0)
    output$go_smw<-renderUI({

      fluidRow(
        column(12,popify(actionButton('downp_we',icon("fas fa-image"),icon("fas fa-download"), style="button_active"),NULL,"download",options=list(container="body"))),
        column(12,
               renderPlot({
                 req(length(smw_dp$df)>1)
                 plot( smw_dp$df, w.effect = TRUE, main="Window size effect")
                 plot_we$df<-recordPlot()

               })
        )
      )
    })
    plot_dp<-reactiveValues(df=0)
    output$plot_dp<-renderPlot({
      smw<- smw_dp$df
      getDP()
      dp_BPs<-if(input$dp_BPs==""){NULL} else{input$dp_BPs}
      dp_w<-if(is.na(input$dp_w)){NULL} else{input$dp_w}
      if(!is.null(input$dp_palette)){
        par(cex=input$dp_cex)
        plot(smw,w=dp_w, sig=input$dp_sig, z=input$dp_z,   BPs=dp_BPs,
             seq.sig=input$dp_seq.sig, bg=getcolhabs(input$dp_palette,nlevels(as.factor(splitBP$df[,1]))),bg_alpha=input$dp_bg,cols=c(input$dp_dcol,input$dp_scol,input$dp_bcol))
      }else{
        plot(smw,w=dp_w, sig=input$dp_sig, z=input$dp_z,   BPs=dp_BPs,
             seq.sig=input$dp_seq.sig, bg=getcolhabs(colors_gradient$val[1],nlevels(as.factor(splitBP$df[,1]))),bg_alpha=0.5,cols=c("black",colors_solid$val[4],colors_solid$val[3]))

      }
      plot_dp$df<-recordPlot()
    })
...
getDP<-reactive({
  ...
  data_empty<-data.frame(attr(saved_data$df[[input$data_upload]],"factors")[,1,drop=F])
  ...
  splitBP$df<- data_empty
  dp
})
    output$dp_extract<-renderUI({
      dp<-getDP()
      fluidRow(
        column(12,style="margin-top: 25px",tipify(actionButton("downcenter_dp_smw",icon("fas fa-download"), icon("fas fa-table")),"Download DP table", options=list(container="body"))),
        column(12,renderPrint({dp})),

        renderPrint({splitBP$df})
      )
    })
    ...
    output$smw_default<-renderUI({
      column(12,
             renderPrint(getpool()))
    })
    splitBP<-reactiveValues(df=F)

    output$save_bp<-renderUI({
      if(!isFALSE(splitBP$df)){
        if(!any(unlist(lapply(attr(saved_data$df[[input$data_upload]],"factors"), function (x) identical(x,as.vector(splitBP$df)))))){
          absolutePanel(style="margin-top: 8px",popify(bsButton("tools_saveBP", icon("fas fa-save"),style  = "button_active", type="action",value=FALSE),"Save breakpoints from DP",
...
output$pop_editDP<-renderUI({
  column(12,splitLayout(cellWidths = c('16%',"16%",'52%','16%'),
                        column(12,
                               strong("Palette"),
                        ...
                        column(12,numericInput("dp_cex", "size",value=1,min=0.1,step=0.1)),
                        column(12,
                               splitLayout(
                                 column(12,
                                        strong("diss col"),
                                        pickerInput(inputId = "dp_dcol",
                                                    ...
                                                    choicesOpt = list(content = colors_solid$img), options=list(container="body"))
                                 ),
                                 column(12,
                                        strong("sig col"),
                                        pickerInput(inputId = "dp_scol",
                                                    label = NULL,
                                                    choices = colors_solid$val,
                                                    choicesOpt = list(content = colors_solid$img), options=list(container="body"), selected=colors_solid$val[4])
                                 ),column(12,
                                          strong("bp col"),
                                          pickerInput(inputId = "dp_bcol",
                                                      label = NULL,
                                                      choices = colors_solid$val,
                                                      choicesOpt = list(content = colors_solid$img), options=list(container="body"), selected=colors_solid$val[3])
                                 )
                               )),
                        column(12,numericInput("dp_bg", "bg",value=0.5,min=0,max=1,step=0.05))
  ))
})


output$segrda_dp<-renderUI({
  validate(need(length(smw_dp$df)>0,"You need to run SMW analysis first"))

  column(12,
         p(strong("Dissimilary Profile")),
         splitLayout(
           numericInput("dp_w",strong('w',tiphelp("A target window size from which results will be extracted. If empty return z-scores averaged over the set of window sizes")), value=NULL),
           selectInput("dp_index",strong('index',actionLink("dp_index_help",tipify(icon("fas fa-question-circle"),"The result to be extracted. Click for details"))),choices=c("dp","rdp","md","sd","oem","osd","params"), selectize=F),
           selectInput("dp_sig",strong('sig',actionLink("dp_sig_help",tipify(icon("fas fa-question-circle"),"Significance test for detecting dissimilarity values that differs significantly from those appearing in a random pattern. Click for details"))),choices=c("z","sd","sd2","tail1"), selectize=F),
           numericInput("dp_z",strong("z",tiphelp("The critical value for the significance of z-values")), value=1.85,step=0.01),
           selectInput("dp_BPs",strong("BPs",tiphelp("Defines if the breakpoints should be chosen as those sample positions corresponding to the maximum dissimilarity in a sequence of significant values (max) or as those sample positions corresponding to the median position of the sequence (median). Defaults to BPs=max. If empty the breakpoints are not computed")),choices=c("","max","median"), selectize=F, selected = "max"),
           numericInput("dp_seq.sig",strong("seq.sig",tiphelp("The maximum length of consecutive, significant values of dissimilarity that will be considered in defining the community breakpoints")), value=3,step=1)),

         column(12,offset=4,uiOutput("save_bp")),
         tabsetPanel(
           tabPanel("DP plot",
                    column(12,
                           popify(actionButton("tools_editDP", icon("fas fa-quidditch"),style  = "button_active"),NULL,"Edit plot",options=list(container="body")),
                           popify(actionButton('downp_dp',icon("fas fa-image"),icon("fas fa-download"), style="button_active"),NULL,"download",options=list(container="body"))


                    ),
                    conditionalPanel("input.tools_editDP % 2",{uiOutput("pop_editDP")}),

                    plotOutput("plot_dp")),
           tabPanel("DP table",
                    column(12,uiOutput("dp_extract"))
    ...
    observeEvent(input$tools_saveBP,{
      hand_save$df<-"Create factor using breakpoints from the dissimilarity profile"
      hand_save2$df<-NULL
      hand_save3$df<-NULL
      showModal(
        hand_save_modal()
    ...
    user_bp_bag<-reactiveValues(df=NULL)
observeEvent(input$inc_bp,{
  user_bp_bag$df<-c(user_bp_bag$df,input$pw_user)
})

output$user_bp<-renderPrint(user_bp_bag$df)

    output$segrda_out<-renderUI({
    ...
    observeEvent(input$run_pwrda,{
      output$summary_pwrda<-renderUI({
        fluidRow(
          column(12,
                 splitLayout(cellWidths = c("60%","25%","15%"),
                             column(12,
                                    column(12,strong("Display"),style="color: #0D47A1"),
                                    column(12,selectInput("disp_segrda_summ",NULL,choices=c('Variable scores','Observation scores','Linear constraints','Biplot','Importance (unconstrained)',"Importance (constrained)"), selectize= F, selected="Importance (unconstrained)"))
                             ),
                             column(12,numericInput("segrda_axes","Number of axes", value=2)),
                             column(12,style="margin-top: 25px",tipify(actionButton("downcenter_segrda",icon("fas fa-download"), icon("fas fa-table")),"Download selected results", options=list(container="body")))

                 ),


          ),
          column(12,verbatimTextOutput("segrda_print")),
          column(12,verbatimTextOutput("pwrda_print"))
    ...
    output$fac_bp<-renderUI({
      fluidRow(
        renderPrint(
          which(!duplicated(sort(attr(saved_data$df[[input$data_upload]],"factors")[,input$pw_sugg])))
        )
      )
    })
    ...
    output$pw_out<-renderUI({
      ...
      column(12,
        ...
        splitLayout(cellWidths = c('25%','45%',"15%","15%"),
                    if(!isFALSE(splitBP$df)){column(12,radioButtons("bp_pwrda","Breakpoints",c("suggested","user-defined"), selected="suggested"))}else{column(12,radioButtons("bp_pwrda","Breakpoints",c("user-defined")))},
                    fluidRow(p(strong("Breaks:"),style="color: #0D47A1"),

                      if(!isFALSE(splitBP$df)){
                        column(12,
                               conditionalPanel("input.bp_pwrda=='suggested'",{
                          renderPrint( bp(getDP()))
                        }))
                      },
                      column(12,conditionalPanel("input.bp_pwrda=='user-defined'",{
                        splitLayout(cellWidths = c('40%','20%','40%'),
                                    numericInput("pw_user", NULL,value=NULL),
                                    span(style="margin-top: 7px",tipify(actionButton("inc_bp",icon("fas fa-arrow-right")),"Type the number of to include a breakpoint")),
                                    verbatimTextOutput("user_bp"))
                      })),
                      column(12,conditionalPanel("input.bp_pwrda=='from Factor-Attribute'",{
                        p(style="margin-top: 50px",
                         splitLayout(cellWidths = c('20%','20%'),
                           selectInput('pw_sugg',NULL,colnames(factors), selected=colnames(factors)[ncol(factors)]),
                           uiOutput("fac_bp")
                         )
                        )
                      }))
                    ),
                    column(12,numericInput('pw_nrand', 'n.rand', value=99)),
                    column(12,style="margin-top: 25px",actionButton("run_pwrda",strong("run pwRDA"), style="button_active"))
...
observeEvent(input$run_pwrda,{
  output$pwRDA_out<-renderUI({
    fluidRow(
      ...
      column(12,
             popify(actionButton("tools_editpw", icon("fas fa-quidditch"),style  = "button_active"),NULL,"Edit plot",options=list(container="body")),
             popify(actionButton('downp_pw',icon("fas fa-image"),icon("fas fa-download"), style="button_active"),NULL,"download",options=list(container="body"))


      ),
      conditionalPanel("input.tools_editpw % 2",{column(12,uiOutput('tools_editpwRDA'))}),
      column(5,
             splitLayout(
               checkboxInput('biplot_segrda', strong("biplot", pophelp(NULL,"show biplot arrows")), T),checkboxInput('sp_segrda', strong("variables", pophelp(NULL,"show variables")), T)
             )),

      column(12,plotOutput("ppwRDA"))
...
observeEvent(input$run_pwrda,{
  output$ppwRDA<-renderPlot({
    ...
    if(length(input$show_symbols_pw)>0){
      segrda=prda(segrda_model()$rda.pw,
                  key = symbol_factor_pw(),
                  points =input$show_symbols_pw,
                  text = input$show_labels_pw,
                  palette = input$colpalette_pw,
                  cex.points = input$cexpoint_pw,
                  cex.text = input$cextext_pw,
                  pch=c(pt_symbol0_pw(),3),
                  textcolor=input$textcolor_pw,
                  keytext=text_factor_pw(),
                  biplot=input$biplot_segrda,
                  show.sp=input$sp_segrda
      )

    }else{
      segrda=prda(segrda_model()$rda.pw, biplot=input$biplot_segrda,show.sp=input$sp_segrda)
    }
    segrda_plot$df<-recordPlot()
    segrda

  })
})

    output$tools_editpwRDA<-renderUI({
      fluidRow(style="border: 3px solid SeaGreen;background: white; margin-bottom: 20px; margin-left: -20px; margin-right: -40px",
               splitLayout(cellWidths = c('58%','42%'),
                           column(12,
                                  ...
                                  uiOutput("symbol_control_pw")
                           ),
                           column(12,
                                  checkboxInput(
                                    'show_labels_pw',
                                    strong("Labels", style="color: #0D47A1;"),F),
                                  splitLayout(style="margin-left: 5px;margin-top: -10px",cellWidths = c('40%','30%','30%'),
                                              uiOutput("controlfactor_pw"),
                                              uiOutput("textsize_pw"),
                                              uiOutput("textcolor_pw")
                                  ))
               )
    ...
    controlfactor_pw <- reactive({
      if (isTRUE(input$show_labels_pw))

        popify(selectInput("factor_pw",
                           NULL,
                           choices = c(colnames(attr(saved_data$df[[input$data_upload]],"factors"))),
                           selectize = F), NULL, "label classification factor",options=list(container="body"))


    })
    output$controlfactor_pw<-renderUI({
      controlfactor_pw()
    })
    output$textsize_pw <- renderUI({
      req(isTRUE(input$show_labels_pw))
      popify(numericInput("cextext_pw",NULL,value = 1,min = 0.1,max = 3,step = .1), NULL, "label text size",options=list(container="body"))


    })
    output$textcolor_pw <- renderUI({
      req(isTRUE(input$show_labels_pw))

      popify(pickerInput(inputId = "textcolor_pw",
                         label = NULL,
                         choices = colors_solid$val,
                         options=list(container="body"),
    ...
    output$symbol_control_pw<-renderUI({
      req(isTRUE(input$show_symbols_pw))
      splitLayout(style="margin-top: -10px",cellWidths = c("20%","20%",'20%','40%'),
                  uiOutput("pt_symbol0_pw"),
                  uiOutput("symbolsize_pw"),
                  uiOutput("palette_mds_pw"),
                  uiOutput("fac_palette_mds_pw")
      )
    })
    output$pt_symbol0_pw<-renderUI({
      popify( pickerInput(inputId = "pt_symbol0_pw",
                          label = NULL,
                          choices = df_symbol$val,
                          options=list(container="body"),
                          choicesOpt = list(content = df_symbol$img)), NULL, "symbol shape",options=list(container="body"))
    })
    output$symbolsize_pw<-renderUI({
      popify(numericInput("cexpoint_pw",NULL,value = 1,min = 0.1,max = 3,step = .1), NULL, "symbol size",options=list(container="body"))
    })
    output$palette_mds_pw<-renderUI({
      popify(pickerInput(inputId = "colpalette_pw",
                         label = NULL,
                         choices = df$val,
                         choicesOpt = list(content = df$img),
                         options=list(container="body"),
                         selected=colors_solid$val[1]
      ), NULL, "Symbol palette. Choose a gradient to use a factor as classification",options=list(container="body"))
    })
    output$fac_palette_mds_pw<-renderUI({
      req(input$colpalette_pw%in%c("matlab.like2",'viridis', 'plasma',"Rushmore1","FantasticFox1","Blues","heat"))
      column(12,
             popify(selectInput("symbol_factor_pw",
                                NULL,
                                choices = c(colnames(attr(saved_data$df[[input$data_upload]],"factors"))),
    ...
    symbol_factor_pw <- reactive({
      if(input$colpalette_pw%in%c("matlab.like2",'viridis', 'plasma',"Rushmore1","FantasticFox1","Blues","heat")){

        data = saved_data$df[[input$data_upload]]
        attr(data,"factors")[rownames(getord()$yo), input$symbol_factor_pw]

      }else{NULL}
    })
    pt_symbol0_pw<-reactive({
      if(isFALSE(input$show_symbols_pw)){NA}else{as.numeric(input$pt_symbol0_pw)}
    })
    text_factor_pw <- reactive({
      if(isFALSE(input$show_labels_pw)){NULL} else{
        data = saved_data$df[[input$data_upload]]
        attr(data,"factors")[rownames(getord()$yo), input$factor_pw]}

    })
    output$segrda_X<-renderUI({
      if(length(saved_data$df)>1){
        fluidRow(
          p(style="margin-left: -20px",span(actionLink("revert_segrda",tipify(icon("fas fa-exchange-alt"),"invert selection")), align="right"),strong("X Data", tiphelp("Response data"))),
          selectInput("segrda_X",NULL, choices=names(saved_data$df)[-which(names(saved_data$df)%in%input$data_upload)], selectize=F)
        )
      }else{em(icon("fas fa-lightbulb"),"you can create a new datalist to use it as a response data")}
    })
...
output$pwrda_print<-renderPrint({
  segrda_model()$summ
})
    segrda_model<-eventReactive(input$run_pwrda,{
      sim1o<-getord()

      model=pwRDA2(sim1o$xo,sim1o$yo , BPs=getBP(),n.rand = input$pw_nrand)
      model
    })
    segrda_plot<-reactiveValues(df=0)

    segrda_summary<-reactive({
      res<-summary(segrda_model()$rda.pw)
      res<-switch(input$disp_segrda_summ,
                  "Variable scores"=res$species,
                  "Observation scores"=res$sites,
                  "Linear constraints"=res$constraints,
                  "Biplot"=res$biplot,
    ...
    observeEvent(input$revert_segrda,{
      cur<-input$data_upload
      updateSelectInput(session,"data_upload",NULL,choices=names(saved_data$df), selected=input$segrda_X)
      updateSelectInput(session,"segrda_X",strong("Y Data", tiphelp("Response data")), choices=names(saved_data$df)[-which(names(saved_data$df)%in%input$data_upload)],selected=cur)
  ...
  output$viewdata<-renderUI({
    fluidRow(
      ...
      if(length(attr(getdata_upload(),"data.factors"))>0){
        column(12,h5(
...
output$shp_feature2<-renderUI({
  ...
  getupload_side<-reactive({
    ...
    fluidRow(
      ...
      div(bsButton(popify(span(icon("fas fa-caret-down"),"Attrs"),"Attributes","",options=list(container="body"), placement="right"),inputId = "submenu_datalist", style = "button_active3", value=F,type ="toggle",block=T
      )),


      div(bsButton(icon=icon("fas fa-caret-right"),popify(span(icon("fas fa-archive")),NULL,"Data-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_data", style = "button_active2", value=F,block=T
      )),
      div(bsButton(icon=icon("fas fa-caret-right"),popify(span(icon("fas fa-boxes")),NULL,"Factor-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_factors", style = "button_active2", value=F,block=T
      )),
      if(!is.null(coords)){
        div(bsButton(icon=icon("fas fa-caret-right"),popify(span(icon("fas fa-map-marker-alt")),NULL,"Coords-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_coords", style = "button_active2", value=F,block=T
        ))
      },
      if(!is.null(base_shape)){
        div(bsButton(icon=icon("fas fa-caret-right"),popify(span('B',icon("fas fa-map")),NULL,"Base-Shape-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_base", style = "button_active2", value=F,block=T
        ))
      },
      if(!is.null(layer_shape)) {

        div(bsButton(icon=icon("fas fa-caret-right"), popify(span('L',icon("fas fa-map")),NULL,"Layer-Shape-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_layer",style = "button_active2", value=F,block=T
        ))
      },
      if(!is.null(som)) {

        div(bsButton(icon=icon("fas fa-caret-right"),popify(span(icon("fas fa-braille")),NULL,"Som-Attribute",options=list(container="body"), placement="right"),inputId = "submenu_som", style = "button_active2", value=F,block=T
        ))
      }
...
bag_submenu<-reactiveValues(df=0)
observeEvent(input$submenu_datalist,{
  output$submenu_out<-renderUI(
    uiOutput("stats_datalist")
  )
})

observeEvent(input$submenu_data,{
  output$submenu_out<-renderUI(
    uiOutput("viewdata")
  )
})

observeEvent(input$submenu_factors,{
  output$submenu_out<-renderUI(
    uiOutput("viewfactors")
  )
})
observeEvent(input$submenu_coords,{
  output$submenu_out<-renderUI(
    uiOutput("viewcoords")
  )
})
observeEvent(input$submenu_base,{
  output$submenu_out<-renderUI(
    uiOutput("viewbase")
  )
})
observeEvent(input$submenu_layer,{
  output$submenu_out<-renderUI(
    uiOutput("viewlayer")
  )
})

observeEvent(input$submenu_som,{
  output$submenu_out<-renderUI(
    uiOutput("viewsom")
  )
})

  observeEvent(input$submenu_agg,{
    updateButton(session,"submenu_datalist",value=F)
    output$submenu_out<-renderUI(
  ...
  output$submenu_tools<-renderUI({

    column(12,
      popify(bsButton("tools_edit", icon("fas fa-quidditch"),style  = "button_active", type="toggle"),NULL,"Edit plot",options=list(container="body")),
      bsButton('downp_box',icon("fas fa-image"),icon("fas fa-download"), style="button_active"),
      conditionalPanel("input.tools_edit % 2",{
        column(12,
               conditionalPanel("input.desc_options=='Boxplot'",{
                 uiOutput("editbox")
               }),
               conditionalPanel("input.desc_options=='PCA'|input.desc_options=='MDS'|input.desc_options=='RDA'||input.desc_options=='segRDA'",{
                 uiOutput("editpca")
               }))
      })
  ...
  output$pop_editplot<-renderUI({
    ...
    column(12,
           if(bag_submenu$df=="boxplot"){ uiOutput("editbox")},
           if(bag_submenu$df=="pca"|bag_submenu$df=="mds"|bag_submenu$df=="rda"|bag_submenu$df=="segrda"){ uiOutput("editpca")}
...
output$tools_bar<-renderUI({
  req(length(saved_data$df)>0)
  if(input$tabs=="menu_upload"){uiOutput('stats_attributes')} else{
  ...
  dataraw <- reactive({
    req(input$up_or_ex)
    if (input$up_or_ex == 'use example data') {
      data <-data.frame(fread("meta/nema_araca.csv", stringsAsFactors = T))
  ...
  observeEvent(input$upload_insert,{
    ...
    if(is.null(saved_data$df)){
      ...
      saved_data$df<-datalist

      if(input$up_or_ex == 'use example data'){
        envi <-data.frame(fread("meta/envi_araca.csv", stringsAsFactors = T))
        ...
        saved_data$df<-c(saved_data$df, envi)
      }
  ...
  pclus_plot<-reactiveValues(df=0)
...
output$pclus_legcontrol<-renderUI({
  ...
  pprop_plot<-reactiveValues(df=0)
  output$pproperty <- renderPlot({
    pproperty(getsom(),
              input$variable_pproperty,
              input$variable_pproperty)
    pprop_plot$df<-recordPlot()
  })
...
colnames(Parameters)<-"Training Parameters"
  ...
  ptree_plot<-reactiveValues(df=0)
  ...
  output$plotoutput <- renderImage({
    ...
    if(hand_plot$df=="boxplot"){replayPlot(pbox_plot$df)}
    if(hand_plot$df=="pca")   {replayPlot(ppca_plot$df)}
    if(hand_plot$df=="mds") {replayPlot(pmds_plot$df)}
    ...
    if(hand_plot$df=="BMUs"){replayPlot(bmus_plot$df)}
    if(hand_plot$df=="property plot") {replayPlot(pprop_plot$df)}
    if(hand_plot$df=="Dendrogram") {replayPlot(pdend_plot$df)}
    ...
    if(hand_plot$df=="codebook clusters"){replayPlot(pclus_plot$df)}
    if(hand_plot$df=="Minimal Depth distribution"){plot(rfd_res$df)}

    if(hand_plot$df=="Multi-way importance"){
      plot(rfm_res$df)
    }
    if(hand_plot$df=="Decision Tree") {replayPlot(ptree_plot$df)}
    if(hand_plot$df=="Map"){plot(map_res$df)}
    if(hand_plot$df=="rda"){replayPlot(rda_plot$df)}
    if(hand_plot$df=="dp"){replayPlot(plot_dp$df)}
    if(hand_plot$df=="we"){replayPlot(plot_we$df)}
    if(hand_plot$df=="segrda"){replayPlot(segrda_plot$df)}
    dev.off()
  ...
  fn_download <- function()
  {
    ...
    switch (hand_plot$df,
      ...
      "boxplot" ={replayPlot(pbox_plot$df)},
      "pca"= {replayPlot(ppca_plot$df)},
      "mds"= {replayPlot(pmds_plot$df)},
      "Training plot"={    pchanges(getsom())},
      "Couting plot"={  pcounts(getsom())},
      "uMatrix"={   pUmatrix(getsom())},
      "BMUs"={replayPlot(bmus_plot$df)},
      "property plot"={replayPlot(pprop_plot$df)},
      "Dendrogram"={replayPlot(pdend_plot$df)},
      ...
      "screeplot_WSS som"={
        elbow_plot(WSSmodel(), sugg = sugg_WSS_model())},
      "Hcut"={hc_plot(phc())},
      "codebook clusters"= {replayPlot(pclus_plot$df)},
      "Minimal Depth distribution"={plot(rfd_res$df)},
      "Multi-way importance"={plot(rfm_res$df)},
      "Decision Tree"={replayPlot(ptree_plot$df)},
      "Map"={plot(map_res$df)},
      "rda"={replayPlot(rda_plot$df)},
      "dp"={replayPlot(plot_dp$df)},
      "we"={replayPlot(plot_we$df)},
      "segrda"={replayPlot(segrda_plot$df)},
